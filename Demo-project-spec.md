# Demo Project Specification: ClozeFlow (ES Module Edition)
...
```js
const db = new Dexie("ClozeFlowDB");
db.version(1).stores({
...
The active session is saved to `localStorage` under the key `"clozeflow_session"` so a page refresh doesn't lose progress.

* **On save:** `localStorage.setItem("clozeflow_session", JSON.stringify(currentSession))` — called every time a chip is placed or removed.
* **On restore:** At startup (inside `initActivity()`), check for a saved session and restore it before rendering.
* **On clear:** `localStorage.removeItem("clozeflow_session")` — called when resetting to idle or moving to graded phase.
...
2. `localStorage.removeItem("clozeflow_session")`.
3. Call `renderIdle()`.
